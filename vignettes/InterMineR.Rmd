...
newConstraint <- list(
  path=c("Gene.pathway.name"),
  op=c("="), 
  value=c("Huntington's disease"), 
  code=c("B")
)
...
Our new filter has been added successfully. Rerun the query and you see you only have one pathway, Huntingdon's disease, returned.
...
You can also add additional columns to the output. For instance, where do these pathway annotations come from? Let's add this information.

Let's see what we know about pathways.

# ```{r add_column, warning=FALSE, message=FALSE}
# model[which(model$type=="Pathway"),]
# ```

The Pathway data type has an 'dataSets' reference of type 'DataSet'.

# ```{r add_column2, warning=FALSE, message=FALSE}
# model[which(model$type=="DataSet"),]
# ```

DataSet has an attribute called "name". Add Gene.pathways.dataSets.name to the view. We'll add it as the last column, we can see from above there are 5 other columns already so we'll put it as #6:
...
queryGenePath$select[[6]] <- "Gene.pathways.dataSets.name"
